...
make_startos_layer() {
  ...
  qemu-img create -f qcow2 -F qcow2 -b \
    "$BVML_STORAGE/canonical/bitcoin-mainnet.qcow2" \
    "$BVML_STORAGE/adapters/startos/bitcoin-mainnet-btrfs.qcow2" >/dev/null
  chmod 0440 "$BVML_STORAGE/adapters/startos/bitcoin-mainnet-btrfs.qcow2"
  printf '%s\n' state=ready id=startos-adapter-id canonical_id=base-id \
    ...
    "backing=$BVML_STORAGE/canonical/bitcoin-mainnet.qcow2" \
...
make_index_base() {
  ...
  printf '%s\n' "id=$service-base-id" "service=$service" \
    ...
    profile_id=indexers-mainnet-v1 "profile_sha256=$profile_sha" \
...
make_stopped_overlay() {
  ...
  assert_file "$BVML_STORAGE/active/ubuntu/bitcoin-mainnet-overlay.qcow2"
}
write_verify() {
...
t_concurrent_consumers() {
  ...
  assert_contains "$TEST_ROOT/attach-ubuntu" "$BVML_STORAGE/active/ubuntu/bitcoin-mainnet-overlay.qcow2"
  assert_contains "$TEST_ROOT/attach-umbrel" "$BVML_STORAGE/active/umbrel/bitcoin-mainnet-overlay.qcow2"
...
t_legacy_layout_migrated() {
  setup_case; trap teardown_case EXIT; make_canonical
  printf 'backing=%s\noverlay\n' "$BVML_STORAGE/canonical/bitcoin-mainnet.qcow2" \
    >"$BVML_STORAGE/active/bitcoin-mainnet-overlay.qcow2"
  printf 'kind=overlay\nvm=ubuntu\noverlay_id=legacy-id\ncanonical_id=base-id\ncheckpoint_generation=base-generation\nbacking=%s\ndisk_serial=legacy-serial\n' \
    "$BVML_STORAGE/canonical/bitcoin-mainnet.qcow2" >"$BVML_STORAGE/active/manifest.env"
  bvml reconcile ubuntu >/dev/null
  assert_absent "$BVML_STORAGE/active/bitcoin-mainnet-overlay.qcow2"
  assert_file "$BVML_STORAGE/active/ubuntu/bitcoin-mainnet-overlay.qcow2"
...
t_failed_attach() { setup_case; trap teardown_case EXIT; make_canonical; touch "$TEST_ROOT/fail-attach"; expect_fail bvml start ubuntu; assert_absent "$BVML_STORAGE/active/ubuntu/bitcoin-mainnet-overlay.qcow2"; assert_absent "$BVML_STORAGE/run/lifecycles/ubuntu/owner.env"; }
t_failed_owner() { setup_case; trap teardown_case EXIT; make_canonical; expect_fail env BVML_FAIL_OWNER_WRITE=1 "$ROOT/bin/bvml" start ubuntu; assert_absent "$TEST_ROOT/attach-ubuntu"; assert_absent "$BVML_STORAGE/active/ubuntu/bitcoin-mainnet-overlay.qcow2"; assert_contains "$TEST_ROOT/virsh.log" 'detach ubuntu'; }
t_failed_start() { setup_case; trap teardown_case EXIT; make_canonical; touch "$TEST_ROOT/fail-start"; expect_fail bvml start ubuntu; assert_absent "$TEST_ROOT/attach-ubuntu"; assert_absent "$BVML_STORAGE/run/lifecycles/ubuntu/owner.env"; assert_absent "$BVML_STORAGE/active/ubuntu/bitcoin-mainnet-overlay.qcow2"; }
t_detach_preserves() { setup_case; trap teardown_case EXIT; make_canonical; bvml start ubuntu >/dev/null; touch "$TEST_ROOT/fail-detach"; expect_fail bvml stop ubuntu; assert_file "$TEST_ROOT/attach-ubuntu"; assert_file "$BVML_STORAGE/run/lifecycles/ubuntu/owner.env"; assert_file "$BVML_STORAGE/active/ubuntu/bitcoin-mainnet-overlay.qcow2"; }
t_reset_active() { setup_case; trap teardown_case EXIT; make_canonical; bvml start ubuntu >/dev/null; bvml reset ubuntu >/dev/null; assert_absent "$BVML_STORAGE/active/ubuntu/bitcoin-mainnet-overlay.qcow2"; assert_eq "$(<"$TEST_ROOT/state-ubuntu")" "shut off"; }
t_reset_inactive() { setup_case; trap teardown_case EXIT; make_canonical; make_stopped_overlay; bvml reset ubuntu >/dev/null; assert_absent "$BVML_STORAGE/active/ubuntu/bitcoin-mainnet-overlay.qcow2"; }
t_reconcile() { setup_case; trap teardown_case EXIT; make_canonical; make_stopped_overlay; id="$(sed -n 's/^overlay_id=//p' "$BVML_STORAGE/run/lifecycles/ubuntu/manifest.env")"; serial="$(sed -n 's/^disk_serial=//p' "$BVML_STORAGE/run/lifecycles/ubuntu/manifest.env")"; printf 'kind=overlay\nvm=ubuntu\nimage=%s\nidentity=%s\ndisk_serial=%s\n' "$BVML_STORAGE/active/ubuntu/bitcoin-mainnet-overlay.qcow2" "$id" "$serial" >"$BVML_STORAGE/run/lifecycles/ubuntu/owner.env"; bvml reconcile >/dev/null; assert_absent "$BVML_STORAGE/run/lifecycles/ubuntu/owner.env"; assert_file "$BVML_STORAGE/active/ubuntu/bitcoin-mainnet-overlay.qcow2"; }
...
t_promote_attached() { setup_case; trap teardown_case EXIT; make_canonical; make_stopped_overlay; write_verify; printf 'vdc %s\n' "$BVML_STORAGE/active/ubuntu/bitcoin-mainnet-overlay.qcow2" >"$TEST_ROOT/attach-umbrel"; expect_fail bvml checkpoint-promote --confirm-synced-clean; }
...
t_bootstrap_stop_preinstall() { setup_case; trap teardown_case EXIT; bvml checkpoint-bootstrap >/dev/null; bvml bootstrap-stop >/dev/null; assert_absent "$BVML_STORAGE/run/lifecycles/ubuntu/owner.env"; assert_absent "$TEST_ROOT/attach-ubuntu"; assert_file "$BVML_STORAGE/active/bitcoin-mainnet-bootstrap.qcow2"; }
...
t_canonical_missing_generation() { setup_case; trap teardown_case EXIT; make_canonical; sed -i '/^generation=/d' "$BVML_STORAGE/canonical/manifest.env"; expect_fail bvml start ubuntu; assert_absent "$BVML_STORAGE/active/ubuntu/bitcoin-mainnet-overlay.qcow2"; }
